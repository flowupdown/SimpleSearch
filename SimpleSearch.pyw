...
class SimpleSearch(Frame):
    def __init__(self, parent=None, startdir='', filename='', browser=None):
        ...
        if self.text == []:
            ...
            for p in self.text:
                ...
                self.text_widget.tag_bind(link, '<Double-1>',
                                lambda event, string=string: subprocess.run([self.browser, string]))
...
if __name__ == '__main__':
    parent = Tk()
    startdir = 'd:\\programming\\python'
    browser = "C:\\Users\\user\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe"
    SimpleSearch(parent, startdir = startdir, browser = browser)
    mainloop()
